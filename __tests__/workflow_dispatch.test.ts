...
test('workflow_dispatch event to slack', async () => {
  ...
  expect(JSON.parse(mockAxios.history.post[0].data)).toStrictEqual({
    ...
    attachments: [
      {
        fallback: '[GitHub]: [step-security/act10ns-slack] manual-test workflow_dispatch Success',
        color: 'good',
        ...
        pretext: '',
        text: '*<https://github.com/step-security/act10ns-slack/actions?query=workflow:%22manual-test%22|Workflow _manual-test_ job _Build and Test_ triggered by _workflow_dispatch_ is _Success_>* for <https://github.com/step-security/act10ns-slack/commits/master|`master`>\n',
        title: '',
        fields: [],
        footer: '<https://github.com/step-security/act10ns-slack|step-security/act10ns-slack> #6',
