...
test('push event to slack', async () => {
  ...
  expect(JSON.parse(mockAxios.history.post[0].data)).toStrictEqual({
    ...
    attachments: [
      {
        fallback: '[GitHub]: [step-security/act10ns-slack] build-test push failure',
        color: 'danger',
        ...
        pretext: '',
        text: '*<https://github.com/step-security/act10ns-slack/actions?query=workflow:%22build-test%22|Workflow _build-test_ job _CI Tests_ triggered by _push_ is _failure_>* for <https://github.com/step-security/act10ns-slack/commits/master|`master`>\n<https://github.com/step-security/act10ns-slack/compare/db9fe60430a6...68d48876e079|`68d48876`> - 4 commits',
        title: '',
        fields: [
          {
            short: false,
